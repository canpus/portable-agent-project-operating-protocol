...
| `ProjectRules/`（4 个文件） | **项目纪律**：只在某一个项目里生效的工作流程（每次干活前要写计划、记状态、留历史……） | 某个班级的"班规" |
| 自动运转的记账系统 | AI 按规则自动在你的项目里建 `tasks/` 目录、项目全景索引（`TASK_INDEX.md`）与文件登记（`FILE_INDEX.md`），写三本账：计划账本、当前状态、历史快照 | 班级的"值日记录本" |
...
└── ProjectRules/
    ├── AGENTS.md                    ← 项目纪律·宪法层（该不该做、往哪走；每会话注入）
    ├── OPERATING_RULES.md           ← 项目纪律·触发纪律层（条件触发：具体怎么做）
    ├── TASK_STATE_MACHINE.md        ← 机制层（状态机：什么时候能做什么、什么时候必须等你批准）
    └── SCHEMA.md                    ← 契约层（账本怎么写、ID 怎么编）
```

- `GlobalRules/AGENTS.md` —— **放一次，所有项目生效**。它是"行为宪法"：禁止幻觉、保护你的已有文件、未经允许不外传数据、不碰系统设置……
- `ProjectRules/` 四个文件 —— **每个项目放一份**。它们定义了"一个 Agent 任务"的完整生命周期：需求确认 → 写计划 → 你批准 → 施工 → 验收 → 留历史，以及三本账（计划/状态/历史）的写法。
...
**这一步做什么**：把 `ProjectRules/` 里的 **4 个文件**复制到你**想管起来的项目**的根目录。放好后，AI 在你这个项目里干活时，就会自动遵守"写计划 → 你批准 → 施工 → 记账"这套流程。

### 6.1 步骤

1. 把 `ProjectRules/` 文件夹里的 **4 个文件全部复制**到你的项目根目录：
   - `AGENTS.md`
   - `OPERATING_RULES.md`
   - `TASK_STATE_MACHINE.md`
   - `SCHEMA.md`
2. **四个文件必须放在同一个文件夹里、互相挨着**（`AGENTS.md` 内部用相对文件名引用另外三个，分开就找不到了）。
3. 如果你的项目根目录已经有 `AGENTS.md`（比如公司/团队已有的规则），不要直接覆盖——把两边的 `AGENTS.md` 内容合并，或者把 `ProjectRules` 四件套放在项目的一个子文件夹里并在已有的 `AGENTS.md` 中加一句引用（具体见 FAQ「项目里已经有 AGENTS.md 怎么办」）。
4. 各工具对项目级文件的读取位置（和全局规则不同，项目级文件名有差异）：

| 工具 | 项目级读取方式 |
|------|---------------|
| ZCode / Codex / Copilot / Cursor | 项目根目录的 `AGENTS.md` 自动读取（四个文件照原样放根目录即可） |
...
5. **什么都不用手动创建**——AI 第一次进入项目干活时，会自动按照规则创建 `tasks/` 目录、三本账，并在项目根生成 `TASK_INDEX.md`（任务全景索引）和 `FILE_INDEX.md`（文件归属登记）。你只需要保证"放对了文件"，剩下的交给规则本身。
...
你的项目/
├── AGENTS.md                        ← 你放的
├── OPERATING_RULES.md               ← 你放的
├── TASK_STATE_MACHINE.md            ← 你放的
├── SCHEMA.md                        ← 你放的
├── TASK_INDEX.md                    ← AI 自动创建（任务全景索引）
├── FILE_INDEX.md                    ← AI 自动创建（文件归属登记）
└── tasks/                           ← AI 自动创建的
    └── 20260819/
        └── task1_XXX/
            ├── 01_inputs/           ← 输入材料
...
**再验一步**（验证"换对话不丢记忆"）：给 AI 一个任务，让它按规则建好 `tasks/` 并干两步，然后**关掉对话，重新开一个**，第一句就问它：

> "我们上一个对话在做什么？现在进行到哪一步了？"

它会先读项目根目录的 `TASK_INDEX.md`（任务全景索引），认出你说的"上一个对话"是哪个任务，然后**询问你是否继续该任务**——你确认后，它打开该任务的 `task_current_state.md`，准确回答出任务编号、当前阶段、下一步。这就是"换对话不丢记忆"的现场演示。
...
1. 新对话的 AI 先读项目根目录 `TASK_INDEX.md`（任务全景索引：项目里有哪些任务、各到哪一步）
2. 用户提到旧任务 → AI 询问是否继续 → 用户确认（双次确认）→ **续接该任务**（不新建任务，任务数量不变）
3. 续接时打开该任务的 `05_docs/task_current_state.md`，拿到任务编号、当前阶段、进度、批准过的计划编号
4. 需要看细节时，去 `plan.md` / `task_history.md` 按编号搜索局部读取（不整文件硬读）
5. 从"安全恢复点"继续干活；若发现疑似并行会话，AI 会主动警告并核实

**换电脑**：把整个项目文件夹复制过去即可——`tasks/`、`TASK_INDEX.md`、`FILE_INDEX.md` 和四件套规则文件都是记忆的一部分，新机器上的 AI 照常读索引、续接或开新任务。
...
**Q2：这套规则会乱动我的代码吗？**
不会。规则要求 AI：修改项目文件前必须经过"计划 → 你批准"；未经批准只能做只读调查；已有文件必须最小修改、保护你的原有内容。账本全部写在 `tasks/` 里，和你的项目文件分开。
...
不冲突，但别直接覆盖别人的规则。推荐做法：把 `ProjectRules` 四件套放到项目根目录下一个子文件夹（比如 `docs/agent-rules/`），然后在已有的 `AGENTS.md` 末尾加一行引用："另见 `docs/agent-rules/` 中的规则，与本文件同等效力"。多数工具支持在项目内任意层级读取规则文件。

**Q6：默认时区是 Asia/Shanghai，我能改吗？**
能。打开项目根目录的 `AGENTS.md`，第 6 行写着"默认项目时区：Asia/Shanghai"，改成你所在时区即可（比如 `UTC+8` 换 `UTC-5` 之类）。改完新开会话生效。

**Q7：换电脑 / 换机器怎么办？**
全局宪法：按 5.2 节重新放置到新电脑的对应位置。项目记忆：把整个项目文件夹（含 `tasks/`、`TASK_INDEX.md`、`FILE_INDEX.md`）复制过去即可，账本文件本身就是记忆。新机器上 AI 读 `TASK_INDEX.md` 与 `task_current_state.md` 就能无缝续接。
